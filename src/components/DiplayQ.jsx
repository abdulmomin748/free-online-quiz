    ...
    return (
        ...
        <div className='shadow-2xl rounded-md p-5 mb-12'>
            <div className='flex justify-between items-start mb-5 text-center'>
                ...
                <button title='Show Correct Answser' className='w-10 flex justify-end cursor-pointer h-10 items-center' onClick={() => handleCorrectAns()}><svg className='eye-svg w-6 h-6' xmlns="http://www.w3.org/2000/svg" fill="#ffb671" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
            ...
            <div className='grid grid-cols-2 gap-2 option-cntlr'>
                {
                    options.map(uniqOption => <Option quizDis={quizDis} key={Option.id} hanldleQuizAns={hanldleQuizAns} uniqOption={uniqOption} />)
                }
            </div>
        </div>
    );
